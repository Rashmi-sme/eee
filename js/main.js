...
/* -------------------------------------------
  ...
  document.querySelectorAll(".mil-has-children > a").forEach((link) => {
    link.addEventListener("click", function (event) {
      ...
      event.preventDefault(); // Ð”Ð¾Ð´Ð°Ñ”Ð¼Ð¾, Ñ‰Ð¾Ð± ÑƒÐ½Ð¸ÐºÐ½ÑƒÑ‚Ð¸ Ð¿ÐµÑ€ÐµÑ…Ð¾Ð´Ñƒ Ð·Ð° Ð¿Ð¾ÑÐ¸Ð»Ð°Ð½Ð½ÑÐ¼
  ...
  window.addEventListener("scroll", () => {
    ...
    if (scrollTop > 100) {
      header.classList.add("mil-transparent");
    } else {
      header.classList.remove("mil-transparent");
    }
...
/* -------------------------------------------
  ...
  document.querySelectorAll(".mil-scroll-to-section").forEach((link) => {
    link.addEventListener("click", function (event) {
      event.preventDefault();
      const targetId = this.getAttribute("href");
      const targetElement = document.querySelector(targetId);
      ...
      const distance = Math.abs(targetPosition - currentPosition);
      const baseDuration = 0.1; // Base duration in seconds
      const duration = baseDuration + distance / 2000; // Adjust this factor as needed
    ...
    document.querySelectorAll(".mil-main-menu li a").forEach((link) => {
      link.addEventListener("click", function (event) {
        ...
        if (isValidHref(href)) {
          ...
          event.preventDefault(); // Ð¯ÐºÑ‰Ð¾ href Ð½ÐµÐ²Ð°Ð»Ñ–Ð´Ð½Ð¸Ð¹, Ð·ÑƒÐ¿Ð¸Ð½ÑÑ”Ð¼Ð¾ Ð´Ñ–ÑŽ Ð·Ð° Ð·Ð°Ð¼Ð¾Ð²Ñ‡ÑƒÐ²Ð°Ð½Ð½ÑÐ¼
        }
      });
    });
...
document.addEventListener("DOMContentLoaded", () => {
  const tabs = document.querySelectorAll(".location-section__tab-btn");
  const panes = document.querySelectorAll(".location-section__tab-pane");

  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      // Remove active class from all tabs
      tabs.forEach((t) =>
        t.classList.remove("location-section__tab-btn--active")
      );

      // Add active class to clicked tab
      tab.classList.add("location-section__tab-btn--active");

      // Hide all tab panes
      panes.forEach((pane) =>
        pane.classList.remove("location-section__tab-pane--active")
      );
      ...
      document
        ...
        .classList.add("location-section__tab-pane--active");
    });
  });
});
...
document.addEventListener("DOMContentLoaded", () => {
  const navs = document.querySelectorAll(".pricing-section__tab-btn");
  const tabPane = document.querySelectorAll(".pricing-section__tab-pane");

  navs.forEach((tab) => {
    tab.addEventListener("click", () => {
      // Remove active class from all navs
      navs.forEach((t) =>
        t.classList.remove("pricing-section__tab-btn--active")
      );

      // Add active class to clicked tab
      tab.classList.add("pricing-section__tab-btn--active");

      // Hide all tab tabPane
      tabPane.forEach((pane) =>
        pane.classList.remove("pricing-section__tab-pane--active")
      );
      ...
      document
        .getElementById(target)
        .classList.add("pricing-section__tab-pane--active");
    });
  });
});
...
document.addEventListener("DOMContentLoaded", () => {
  const navs = document.querySelectorAll(".agenda-section__tab-btn");
  const tabPane = document.querySelectorAll(".agenda-section__tab-pane");

  navs.forEach((tab) => {
    tab.addEventListener("click", () => {
      // Remove active class from all navs
      navs.forEach((t) =>
        t.classList.remove("agenda-section__tab-btn--active")
      );

      // Add active class to clicked tab
      tab.classList.add("agenda-section__tab-btn--active");

      // Hide all tab tabPane
      tabPane.forEach((pane) =>
        pane.classList.remove("agenda-section__tab-pane--active")
      );
      ...
      document
        .getElementById(target)
        .classList.add("agenda-section__tab-pane--active");
    });
  });
});
...
document.addEventListener("DOMContentLoaded", () => {
  const navs = document.querySelectorAll(".sponsors-section__tab-btn");
  const tabPane = document.querySelectorAll(".sponsors-section__tab-pane");

  navs.forEach((tab) => {
    tab.addEventListener("click", () => {
      // Remove active class from all navs
      navs.forEach((t) =>
        t.classList.remove("sponsors-section__tab-btn--active")
      );

      // Add active class to clicked tab
      tab.classList.add("sponsors-section__tab-btn--active");

      // Hide all tab tabPane
      tabPane.forEach((pane) =>
        pane.classList.remove("sponsors-section__tab-pane--active")
      );
      ...
      document
        .getElementById(target)
        .classList.add("sponsors-section__tab-pane--active");
    });
  });
});

// document.addEventListener('DOMContentLoaded', function () {
//   const transitionFade = document.querySelector('.mil-transition-fade');

//   if (transitionFade) {
//     // Function to set height to auto
//     const setHeightAuto = () => {
//       transitionFade.style.height = 'auto';
//     };
...
//         setTimeout(() => {
//           isThrottled = false;
//         }, 100); // Adjust delay as needed (100ms for smoother performance)
//       }
//     });
//   }
// });